...
TotHits=0
TotFailures=0
with open( "CarColorResultsWithoutHistogra.txt" ,"w") as  w:
    for i in range (len(imagesComplete)):
            ...
            for j in range( len(TabImgSelect)):
                 ...
                 #print(TabImgSelect[j].shape)
               
                 #y_center = y[j] + int ((yMax[j] - y[j])/2)
                 #x_center= x[j] + int ((xMax[j] - x[j])/2)

                 y_center = int( height/2)
                 x_center = int(width / 2)

                 print ( "ROI point, y_center = "+ str(y_center)+ " x_center = "+ str(x_center))
                 ...
                 cv2.circle(img,(x_center, y_center), 10, (0,255,0), thickness=5)
                 cv2.imshow("ROI", img)  
                 cv2.waitKey(0)

                 #https://stackoverflow.com/questions/12187354/get-rgb-value-opencv-python
                 B = img[y_center , x_center , 0]
                 G = img[y_center , x_center , 1]
                 R = img[y_center , x_center , 2]
                 ...
                 Y_elected=modelKNN.predict(X_test)
